...
const MessageInput = ({ messageTyped, setMessageType, currentUserId, chatOpened, socket, setMessages, setMessageTyped }: any) => {
    ...
    const sendMsg = async (event: MouseEvent<HTMLFormElement>) => {
        ...
        if (messageTyped.length || key.length) {
            const response = await fetch("http://localhost:8000/message/send", {
                ...
                body: JSON.stringify({ sender: currentUserId, message: messageTyped, reciever: chatOpened,awsFileKey:key, awsFileType:fileType })
            })
    ...
    const fileInput = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedFile) {
            ...
            if (selectedFile.size > maxSizeInBytes) {
                e.target.value = '';
                alert("Please choose a file below 20MB")
                return;
            } else {
    ...
    return (
        <div>
            {fileSelectedName.length ? <div className='w-full bg-white flex gap-3 p-3 mb-0 items-center'>
                {!sending ? <>
                ...
                </>   : <p className='text-sm'>Sending your file........</p> }            
            </div> : <div className=''>
                    
            </div>}
            <form className="relative shadow-sm" onSubmit={sendMsg}>
                <input value={messageTyped} autoComplete="false" type="text" id="price" className="w-full p-3 pr-28 text-sm border-l border-slate-400 border-opacity-30 outline-none" placeholder="Enter your Message" onChange={(e) => setMessageTyped(e.target.value)} />
